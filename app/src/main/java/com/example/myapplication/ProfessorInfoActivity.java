...
public class ProfessorInfoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.pInfoButton).setOnClickListener(onClickListener);
    }

    View.OnClickListener onClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if (v.getId() == R.id.pInfoButton) {
                UpdateInfo();
            }
        }
    };
    ...
    private void UpdateInfo() {
        String number = ((EditText)findViewById(R.id.pNumberEditText)).getText().toString();
        String name = ((EditText)findViewById(R.id.pNameEditText)).getText().toString();
        String major = ((EditText)findViewById(R.id.pMajorEditText)).getText().toString();
        ...
        if(name.length()>0 && number.length()>0 && major.length()>0) {
            ...
            db.collection("professor").document(user.getUid()).set(memberInfo)
            ...
            categorize.put("category", "professional");
